...
# MARK: - iOS Static Framework
  ...
  s.subspecs.each do |sc|
    cn = sc.consumer(:ios)
    sc.source_files = cn.source_files.map { |pt| "#{cn.version}/#{pt}" }
    sc.public_header_files = cn.public_header_files.map { |pt| "#{cn.version}/#{pt}" }
    sc.resources = cn.resources.map { |pt| "#{cn.version}/#{pt}" }
    sc.exclude_files = cn.exclude_files.map { |pt| "#{cn.version}/#{pt}" }
    sc.requires_arc = cn.requires_arc.map { |pt| "#{cn.version}/#{pt}" }
  end
end
